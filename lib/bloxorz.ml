...
module Block : sig
  ...
  let right = function
    | Standing { row; col } ->
        Horizontal ({ row; col = col + 1 }, { row; col = col + 2 })
    | Horizontal ({ row; _ }, { col = c2; _ }) -> Standing { row; col = c2 + 1 }
    | Vertical ({ row = r1; col }, { row = r2; _ }) ->
        Vertical ({ col = col + 1; row = r1 }, { col = col + 1; row = r2 })

  let left = function
    | Standing { row; col } ->
        Horizontal ({ row; col = col - 2 }, { row; col = col - 1 })
  ...
  let up = function
    | Standing { row; col } ->
        Vertical ({ row = row - 2; col }, { row = row - 1; col })
    | Horizontal ({ row; col = c1 }, { col = c2; _ }) ->
        Horizontal ({ row = row - 1; col = c1 }, { row = row - 1; col = c2 })
    | Vertical ({ row = r1; col }, _) -> Standing { col; row = r1 - 1 }

  let down = function
    | Standing { row; col } ->
        Vertical ({ row = row + 1; col }, { row = row + 2; col })
    | Horizontal ({ row; col = c1 }, { col = c2; _ }) ->
        Horizontal ({ row = row + 1; col = c1 }, { row = row + 1; col = c2 })
  ...
  let moves b =
    [ (right b, `Right); (left b, `Left); (up b, `Up); (down b, `Down) ]
end

module TerrainBlock = struct
  type t = [ `Empty | `Block | `Glass | `Button of int | `Start | `Goal ]
  [@@deriving show, eq, ord]
...
module Terrain : sig
  ...
  type t = { terrain : terrain; buttons : button_action B.t }
  ...
  let push_button ({ buttons; _ } as t) id block =
    match B.find_opt id buttons with Some f -> f block t | None -> t
  ...
  let equal { terrain = t1; _ } { terrain = t2; _ } =
    M.equal TerrainBlock.equal t1 t2

  let compare { terrain = t1; _ } { terrain = t2; _ } =
    M.compare TerrainBlock.compare t1 t2

  let get ~pos { terrain; _ } =
    Option.value (M.find_opt pos terrain) ~default:`Empty
  ...
  module Parser = struct
    ...
    let tb =
      any_char >>| fun c -> if c = 't' then `Toggle else TerrainBlock.of_char c
    ...
    let req =
      ...
      >>= (function
            | 'S' -> return Standing
            | 'A' -> return All
            | _ -> fail "requirement must be either 'S' or 'A'")
      <* advance 1
    ...
    let pos =
      ...
      int <* char ')' >>= fun col -> return (Pos.make ~row ~col)

    let value = (fun pos block -> (pos, block)) <$> pos <* char '=' <*> tb
    ...
    let get_button_fun updates req block terrain =
      let rec add_block terrain (pos, block) =
        ...
        | `Toggle ->
            let new_block =
              match get terrain ~pos with `Empty -> `Block | _ -> `Empty
            in
            add_block terrain (pos, new_block)
        | #TerrainBlock.t as block -> update terrain ~pos ~block
      in
      if req_fullfilled req block then List.fold_left add_block terrain updates
      else terrain
    ...
    let rec parse_buttons lines =
      ...
      | hd :: tl -> (
          ...
          | Ok (id, f) -> Result.map (fun t -> B.add id f t) (parse_buttons tl)
          | Error s -> Error s)
  ...
  let of_string_exn str =
    match of_string str with Ok v -> v | Error s -> failwith s

  let find_block block t =
    M.bindings t |> List.find (fun (_, b) -> b = block) |> fst
  ...
  let show { terrain = t; _ } =
    Seq.map
      (fun (pos, block) ->
        Printf.sprintf "%s : %s" (Pos.show pos) (TerrainBlock.show block))
      (M.to_seq t)
...
module GameDef : sig
  ...
  module State = struct
    ...
    let initial terrain =
      {
        current = Block.at @@ Terrain.start terrain;
        terrain;
        moves = [];
    ...
    let at_goal { current; goal; _ } =
      match current with Standing pos -> Pos.equal pos goal | _ -> false
